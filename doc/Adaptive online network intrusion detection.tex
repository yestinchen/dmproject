...
\begin{document}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\IF {$size(p_\text{buffer}) < w$}
			\STATE {append $x_i$ to $p_\text{buffer}$}
			\ELSE 
			\STATE {group data records with same label together in $p_\text{buffer}$ } 
			\STATE {$clustered\_partition \gets empty$ }
			\FOR {$label, record\_list$ in $p_\text{buffer}$}
