...
\begin{document}
	...
	\begin{abstract} \label{abstract}
		...
		Online network intrusion detection has been an important issue in both stream data mining and network security. How to identify intrusions with fast and effective algorithms is the key to most of IDS systems. In this paper, first we give an overview of online network intrusion detection systems, containing data collection, feature extraction, normalization and online algorithm. We further developed a new method to detect network intrusions using label classification. Two main steps are performed in the method, namely classification and data reduction. k-NN method is applied in classification phase while clustering techniques are introduced during data reduction. We first group records with same label together, and using cluster centers instead of raw data records.  Experiments show that our method has better accuracy comparing with ECSMiner and k-NN. The influence of different values for parameters are also studied in this paper. 
		
	\end{abstract}
	
	\section{Introduction} \label{introduction}
	
	Large amount of network traffic data is generated at a rapid rate every day, since computers are everywhere in our daily life. Meanwhile, such wide usage of Internet also increases the potential damage that can be caused by attacks. It is increasingly difficult to employ traditional intrusion detection methods to prevent attacks with ever-growing proliferation of new types of attacks. 
	...
	Many data stream classification algorithms have been proposed to address two major challenges relating to data streams: infinite length and concept-drift. Data streams can run indefinite and thus have infinite length. Therefore, a learning algorithm that requires multiple passes over a dataset is unsuitable, since it would require infinite storage and training time \cite{Siboni2014}. Concept-drift, in which the underlying concept of the data changes over the time, requires that the algorithm periodically update the model. For instance, changes in online user behaviour might result in changes to the network traffic data collected over time. These changes could impact the performance of our models if they are not updated accordingly. 
	
	
	Another challenge that has yet to be address by many data stream classification algorithms is dealing with concept-evolution. Concept-evolutions means identifying the emergence of a novel class. This is particularly pertinent for intrusion detection where new types of attacks appear on a regular basis and the ability to identify new types of attacks early would be extremely advantageous. 
	
	Some researchers have proposed some hybrid models to combine both offline and online data that address this issue \cite{Siboni2014}\cite{Masud2013}. For this paper, we propose a new data stream classification algorithm based on \cite{Siboni2014}. Our technique involves partitioning the data stream into equal-sized chunks, that each can be accommodated in memory and processed online. Each chunk is used to train one classification model as soon as all the instances in the chunk are labeled.
	
	In this paper, we present an overview of online network intrusion detection systems. Labels are used to identify different type of intrusions. An online learning algorithm further been developed to improve the accuracy of results. The rest of this paper will be 
